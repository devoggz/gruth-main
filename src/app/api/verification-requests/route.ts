...
export async function POST(req: NextRequest) {
  try {
    ...
    const descriptionWithCoords = latitude && longitude
    ...
    const request = await prisma.verificationRequest.create({
      data: {
        ...
        filesJson: uploadedFiles?.length
    ...
    try {
      await resend.emails.send({
        ...
        html:    adminNotificationHtml(parsed.data, request.id, uploadedFiles ?? []),
      });
    } catch (emailErr) {
        ...
        <tr>
          <td style="padding:28px 32px;">
            <table width="100%" cellpadding="0" cellspacing="0">
              ${[["Name", data.name],["Email", data.email],["Phone", data.phone ?? "—"],["Location", data.projectLocation],["County", data.county ?? "—"],["Service", data.serviceType],["Urgency", data.urgency ?? "standard"]]
      .map(([l, v]) => `<tr><td style="padding:6px 0;color:#9ca3af;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;width:110px;vertical-align:top;">${l}</td><td style="padding:6px 0;color:#3d3b36;font-size:14px;">${v}</td></tr>`)
      .join("")}
            </table>
