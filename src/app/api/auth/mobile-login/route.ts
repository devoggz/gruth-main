...
export async function POST(req: NextRequest) {
  ...
  if (contentLength && parseInt(contentLength) > 4096) {
    return NextResponse.json({ error: "Payload too large." }, { status: 413, headers: hdrs });
  }

  try {
    const parsed = schema.safeParse(await req.json().catch(() => ({})));
    if (!parsed.success) {
      return NextResponse.json({ error: "Invalid credentials." }, { status: 400, headers: hdrs });
    }
    ...
    if (!user || !user.passwordHash || !ok) {
      return NextResponse.json({ error: "Invalid email or password." }, { status: 401, headers: hdrs });
    }
    ...
    return NextResponse.json({ error: "Login failed." }, { status: 500, headers: hdrs });
  }
}