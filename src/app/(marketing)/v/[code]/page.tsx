...
export default async function VerificationBadgePage({ params }: Props) {
  ...
  const photoCount = project.inspections.reduce(
    (s, i) => s + i.media.length,
    0,
  );
  ...
  const completedDate = latest?.completedDate
    ? new Date(latest.completedDate).toLocaleDateString("en-GB", {
        day: "numeric",
        month: "long",
        year: "numeric",
      })
        ...
        <div className="bg-white/[0.04] border border-white/10 rounded-3xl overflow-hidden mb-6">
          ...
          {firstPhotos.length > 0 && (
            <div className="grid grid-cols-4 gap-px">
              {firstPhotos.map((photo, i) => (
                <div
                  key={photo.id}
                  className={`relative bg-charcoal-800 overflow-hidden ${i === 0 ? "col-span-2 row-span-2 aspect-square" : "aspect-square"}`}
                >
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={photo.url}
              ...
              {firstPhotos.length === 0 && (
                <div className="col-span-4 aspect-[4/1] bg-charcoal-800 flex items-center justify-center">
                  <span className="text-charcoal-600 text-sm">
                    No photos in this verification
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="p-6 sm:p-8 space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${project.status === "COMPLETED" ? "bg-emerald-500" : "bg-orange-500"}`}
                />
                {statusLabel[project.status] ?? project.status}
              </span>
              {completedDate && (
                <span className="text-charcoal-400 text-xs">
                  Verified on {completedDate}
                </span>
              )}
            </div>
            ...
            {latest?.summary && (
              <div>
                <p className="text-[10px] font-bold text-charcoal-500 uppercase tracking-widest mb-2">
                  Inspector Summary
                </p>
                ...
                <div className="flex items-center gap-2">
                  <div className="flex gap-0.5">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <svg
                        key={n}
                        className={`w-4 h-4 ${n <= latest.overallRating! ? "text-orange-400" : "text-charcoal-700"}`}
                        viewBox="0 0 24 24"
                        fill="currentColor"
                      >
                  ...
                  {latest.workQuality && (
                    <span
                      className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                        latest.workQuality === "EXCELLENT"
                          ? "bg-emerald-500/10 text-emerald-400"
                          : latest.workQuality === "GOOD"
                            ? "bg-blue-500/10 text-blue-400"
                            : latest.workQuality === "FAIR"
                  ...
                  {latest.safetyCompliance !== null && (
                    <span
                      className={`text-xs ${latest.safetyCompliance ? "text-emerald-400" : "text-red-400"}`}
                    >
                      {latest.safetyCompliance
                        ? "✓ Safety compliant"
            ...
            <div className="grid grid-cols-2 gap-3 pt-2 border-t border-white/[0.06]">
              {project.inspector && (
                <div>
                  <p className="text-[10px] font-bold text-charcoal-500 uppercase tracking-widest mb-1.5">
                    Inspector
                  </p>
                  <div className="flex items-center gap-2">
