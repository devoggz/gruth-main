...
import {
  ...
  getStatusColor, getProjectTypeLabel,
} from "@/lib/utils";
...
export default async function ProjectDetailPage({ params }: PageProps) {
  ...
  const project = await prisma.project.findFirst({
    ...
    include: {
      ...
      inspections:    { orderBy: { scheduledDate: "desc" }, include: { media: { orderBy: { sortOrder: "asc" } }, report: true } },
  ...
  const progressPct      = project.progressStages.length > 0
      ? Math.round((completedStages / project.progressStages.length) * 100) : 0;
  const upcomingInspections  = project.inspections.filter(i => i.status === "SCHEDULED");
      ...
      <div className="space-y-6 w-full pb-12">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-2 text-sm">
              ...
              <span className="text-charcoal-700 truncate max-w-[200px]">{project.name}</span>
            </div>
              ...
              {[
                ...
                ["Inspector",   project.inspector?.name ?? "Not yet assigned"],
                  ...
                  <div key={k} className="flex justify-between gap-2">
                    ...
                    <dd className="font-medium text-charcoal-800 text-right max-w-[60%] truncate">{v}</dd>
                  </div>
              ))}
            </dl>
          </div>
              ...
              <div className="bg-white border border-charcoal-100 rounded-2xl p-5">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                      progressPct >= 100 ? "bg-emerald-50 text-emerald-700"
                          : progressPct >= 60 ? "bg-orange-50 text-orange-700"
                ...
                <div className="h-2 bg-charcoal-100 rounded-full overflow-hidden mb-4">
                  <div className="h-full rounded-full transition-all duration-700"
                       style={{ width: `${progressPct}%`, background: progressPct >= 80 ? "#16a34a" : "#f97316" }}
                  />
                </div>
                <div className="space-y-2.5">
                  {project.progressStages.map(stage => (
                      <div key={stage.id} className="flex items-center gap-3">
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${stage.completed ? "bg-emerald-500" : "bg-charcoal-100"}`}>
                          {stage.completed && (
                        ...
                        <span className={`text-sm flex-1 ${stage.completed ? "text-charcoal-800 font-medium" : "text-charcoal-400"}`}>
                    {stage.stageName}
                  </span>
                        {stage.completedAt && (
                            <span className="text-xs text-charcoal-300">{formatDate(stage.completedAt)}</span>
                        )}
                      </div>
                  ))}
                </div>
        ...
        {upcomingInspections.length > 0 && (
            <div className="bg-orange-50 border border-orange-200 rounded-2xl p-5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-semibold text-charcoal-900 text-sm">Upcoming Inspection</p>
                  <p className="text-charcoal-600 text-xs mt-0.5">
                    {formatDate(upcomingInspections[0].scheduledDate)} · {formatRelativeDate(upcomingInspections[0].scheduledDate)} · {upcomingInspections[0].inspectorName}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white border border-charcoal-100 rounded-2xl p-5 sm:p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="font-display font-semibold text-charcoal-950">Budget Tracker</h2>
        ...
        {/* ── Inspection Reports ─────────────────────────────────────────────── */}
        <div className="bg-white border border-charcoal-100 rounded-2xl p-5 sm:p-6">
          <h2 className="font-display font-semibold text-charcoal-950 mb-5">Inspection Reports</h2>
          {project.inspections.length === 0 ? (
              <p className="text-charcoal-400 text-sm">
                No inspections yet. Your inspector will submit reports after each visit.
              </p>
          ) : (
                  ...
                  {project.inspections.map(inspection => (
                      ...
                      {inspection.status}
                        ...
                        {inspection.observations && (
                            <div className="mt-3">
                              <p className="text-[10px] text-charcoal-400 font-bold uppercase tracking-widest mb-1.5">Observations</p>
                        ...
                        {inspection.recommendations && (
                            <div className="mt-3">
                              <p className="text-[10px] text-charcoal-400 font-bold uppercase tracking-widest mb-1.5">Recommendations</p>
                        ...
                        {inspection.overallRating && (
                            <div className="flex items-center gap-3 mt-3 flex-wrap">
                              <div className="flex gap-0.5">
                                {[1,2,3,4,5].map(n => (
                                    <svg key={n} className={`w-4 h-4 ${n <= inspection.overallRating! ? "text-orange-400" : "text-charcoal-200"}`}
                              ...
                              {inspection.workQuality && (
                                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                                      inspection.workQuality === "EXCELLENT" ? "bg-emerald-50 text-emerald-700"
                                          : inspection.workQuality === "GOOD"    ? "bg-blue-50 text-blue-700"
                                              : inspection.workQuality === "FAIR"    ? "bg-amber-50 text-amber-700"
                        ...
                        {inspection.nextSteps && (
                            <div className="mt-3 flex items-start gap-2 text-sm text-charcoal-600">
                              <span className="text-orange-500 font-semibold flex-shrink-0">→</span>
                              <span>{inspection.nextSteps}</span>
                            </div>
                        )}

                        {inspection.media.filter(m => m.type === "PHOTO").length > 0 && (
                            <div className="flex gap-2 mt-4 overflow-x-auto pb-1">
                              {inspection.media.filter(m => m.type === "PHOTO").map(media => (
                                  <a key={media.id} href={media.url} target="_blank" rel="noreferrer"
                                    ...
                                    <Image src={media.url} alt={media.caption ?? media.filename} fill
                        ...
                        {inspection.report?.fileUrl && (
                            <a href={inspection.report.fileUrl} target="_blank" rel="noreferrer"
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {allPhotos.map(photo => (
                    <a key={photo.id} href={photo.url} target="_blank" rel="noreferrer"
                      ...
                      <Image src={photo.url} alt={photo.caption ?? photo.filename} fill
        ...
        {project.materialPrices.length > 0 && (
            <div className="bg-white border border-charcoal-100 rounded-2xl p-5 sm:p-6 overflow-x-auto">
              <h2 className="font-display font-semibold text-charcoal-950 mb-5">Material Price Intelligence</h2>
              <table className="w-full text-sm min-w-[600px]">
                <thead>
                <tr className="border-b border-charcoal-100">
                  {["Material","Unit","Market Range","Quoted","Verified","Status"].map(h => (
                      <th key={h} className="text-left text-[10px] font-bold text-charcoal-400 uppercase tracking-widest py-2.5 pr-4 whitespace-nowrap">
                        {h}
                      </th>
                  ))}
                </tr>
                ...
                {project.messages.slice(-6).map(msg => (
                    <div key={msg.id} className={`flex ${msg.isFromClient ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                        ...
                        {!msg.isFromClient && (
                            <div className="text-[10px] font-bold text-orange-500 mb-1 uppercase tracking-widest">GRUTH Team</div>
                        )}
                        {msg.content}
                        <div className={`text-xs mt-1.5 ${msg.isFromClient ? "text-white/40" : "text-charcoal-400"}`}>
