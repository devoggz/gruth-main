...
> = {
  ...
  CLOSED: {
    label: "Closed",
    dot: "bg-charcoal-300",
    badge: "bg-charcoal-50 text-charcoal-500 ring-charcoal-200",
  },
};
...
function StatusBadge({ status }: { status: RequestStatus }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ring-1 ${c.badge}`}
    >
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <div className="w-full max-w-lg bg-white h-full shadow-2xl flex flex-col overflow-hidden">
        <div className="flex items-start justify-between px-6 py-5 border-b border-charcoal-100">
          <div>
            <h3 className="font-display font-bold text-charcoal-950 text-lg">
              {request.name}
            </h3>
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-charcoal-100 transition-colors text-charcoal-400 hover:text-charcoal-700"
          >
            <svg
              width="16"
              height="16"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className="bg-charcoal-50 rounded-xl p-3.5">
                ...
                {href ? (
                  <a
                    href={href}
                    className="text-sm font-medium text-orange-600 hover:text-orange-700 truncate block"
                  >
                    {value}
                  </a>
                ) : (
          ...
          {/* Description */}
          <div>
            <p className="text-xs font-bold text-charcoal-400 uppercase tracking-wider mb-2">
              Description
            </p>
          ...
          {isConverted && convertedProjectId ? (
            ...
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
              <p className="font-semibold text-emerald-800 text-sm">
                This request has been converted to a project.
              </p>
              <Link
                href="/admin/projects"
                className="text-sm text-emerald-700 hover:text-emerald-900 underline mt-1 inline-block"
              >
                View all projects →
              </Link>
            </div>
            ...
            <div className="border border-charcoal-200 rounded-xl p-4 space-y-3">
              <p className="text-xs font-bold text-charcoal-400 uppercase tracking-wider">
                Convert to Project
              </p>
              <p className="text-xs text-charcoal-500">
                Creates a live project + client account (if not existing).
                Optionally assign an inspector immediately.
              </p>
              ...
              {convertError && (
                <p className="text-red-600 text-xs">{convertError}</p>
              )}
        ...
        {/* Footer actions */}
        <div className="px-6 py-4 border-t border-charcoal-100 flex gap-2.5">
          <a
          ...
          {request.phone && (
            <a
              href={`https://wa.me/${request.phone.replace(/\D/g, "")}?text=${encodeURIComponent(`Hi ${request.name.split(" ")[0]}, this is GRUTH regarding your ${request.serviceType} verification request.`)}`}
              target="_blank"
              rel="noreferrer"
  ...
  return (
    <>
      <div className="bg-white rounded-xl border border-charcoal-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-charcoal-100 flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-charcoal-400 mt-0.5">
              {requests.length} total from the intake form
            </p>
          </div>
        ...
        {loading ? (
          <div className="px-6 py-16 text-center text-charcoal-400 text-sm">
            Loading requests…
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div className="divide-y divide-charcoal-50">
            {filtered.map((req) => (
              <button
                ...
                className={`w-full text-left px-6 py-4 hover:bg-orange-50/30 transition-colors group flex items-start gap-4 ${
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-charcoal-500 mt-1.5 line-clamp-1">
                    {req.description}
                  </p>
                </div>
