...
const testimonials = [
  {
    quote:    "I was being told my house was 70% done. GRUTH visited and confirmed the walls were only halfway up. I would have sent another £15,000 for work that hadn't happened.",
    name:     "Sarah O.",
  ...
  {
    quote:    "They visited the plot I was about to buy and found it was already being claimed by someone else. There was an active dispute I knew nothing about. I would have lost everything.",
    name:     "Michael K.",
  ...
  {
    quote:    "My contractor was quoting cement at nearly 30% above market. GRUTH's material pricing report gave me the exact figures to push back with. He adjusted the quote immediately.",
    name:     "Grace N.",
  ...
  {
    quote:    "The caretaker hadn't been to the property in months. GRUTH confirmed the fence had been removed and there was an encroachment on the eastern side. We'd never have known.",
    name:     "David M.",
  ...
  {
    quote:    "The wedding event oversight was brilliant. I couldn't travel back and my aunt kept saying everything was fine. GRUTH's agent was on the ground from setup to close — real-time updates.",
    name:     "Wanjiku A.",
  ...
  {
    quote:    "I wanted to invest in my brother's shop but couldn't verify it was real. GRUTH confirmed the premises, checked the stock, and confirmed the trading history. I invested with confidence.",
    name:     "Peter O.",
    location: "Dubai, UAE",
    flag:     "🇦🇪",
...
function StarRating({ count }: { count: number }) {
  return (
      <div className="flex gap-0.5">
        {[1,2,3,4,5].map(n => (
            <svg key={n} className={`w-3.5 h-3.5 ${n <= count ? "text-orange-400" : "text-charcoal-700"}`}
...
export default function TestimonialsSection() {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(
        (entries) => entries.forEach(e => { if (e.isIntersecting) { (e.target as HTMLElement).classList.add("ts-in"); obs.unobserve(e.target); } }),
        { threshold: 0.08 }
    );
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <span className="inline-flex items-center gap-2 text-orange-400 text-[10px] font-bold tracking-widest uppercase bg-orange-500/10 border border-orange-500/20 px-3 py-1.5 rounded-full mb-5">
            ...
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-white mb-4 tracking-tight">
              Real projects. Real money.<br className="hidden sm:block"/>
              <span className="text-orange-400"> Real results.</span>
            </h2>
            <p className="text-charcoal-400 text-base max-w-lg mx-auto">
              Every card below is a situation where distance was about to cost someone dearly. GRUTH changed the outcome.
            </p>
          </div>

          {/* Cards grid */}
          <div ref={gridRef} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-12 ts-fade">
            {testimonials.map(({ quote, name, location, flag, service, saved, rating }) => (
                <div key={name}
                  ...
                  {saved && (
                      <div className="bg-orange-500/8 border border-orange-500/15 rounded-xl px-3 py-2 flex items-center gap-2">
                        <svg className="w-3.5 h-3.5 text-orange-400 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  ...
                  <div className="border-t border-white/[0.06] pt-4 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-charcoal-500 text-[10px]">{location}</p>
                      </div>
                    </div>
