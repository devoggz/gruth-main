      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={headRef} className="text-center mb-14 comm-fade">
          ...
          <p className="text-charcoal-400 text-base max-w-xl mx-auto leading-relaxed">
            Every week, thousands of Kenyans abroad discuss properties,
            contractors, and projects. GRUTH is where those conversations turn
            into verified answers.
          </p>
        </div>
        ...
        <div
          ref={statsRef}
          className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-14 comm-fade"
        >
          {STATS.map(({ value, label }) => (
            <div
              key={label}
              className="bg-white/[0.04] border border-white/[0.07] rounded-2xl p-5 text-center"
            >
              <div className="font-display text-3xl font-bold text-white mb-1">
                {value}
              </div>
              <div className="text-[11px] text-charcoal-500 leading-snug">
                {label}
              </div>
            </div>
          ))}
        ...
        <div
          ref={cardsRef}
          className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-14 comm-fade"
        >
          {CHANNELS.map(
            ({ icon, label, desc, cta, href, color, border, bg }) => (
              <div
                key={label}
                className={`${bg} border ${border} rounded-2xl p-5 flex flex-col gap-4`}
              >
          ...
          <div className="bg-white/[0.03] border border-white/[0.07] rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="text-center sm:text-left">
              ...
              <p className="text-charcoal-400 text-sm leading-relaxed max-w-md">
                Refer a friend who completes a verification and earn yourself a
                free one. Your community trusts you — help them trust their
                projects.
              </p>
            </div>
