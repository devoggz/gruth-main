...
const SERVICE_OPTIONS = [
  { value: "Construction Verification",           label: "🏗️  Construction Verification"           },
  { value: "Land & Property Verification",        label: "🏡  Land & Property Verification"        },
  { value: "Wedding / Event Verification",        label: "💒  Wedding / Event Verification"        },
  { value: "Funeral Event Oversight",             label: "🕊️  Funeral Event Oversight"            },
  { value: "Business & Investment Verification",  label: "📊  Business & Investment Verification" },
  { value: "Material Price Intelligence",         label: "🧱  Material Price Intelligence"        },
...
// Steps differ based on whether the user is logged in
// Logged-in + complete profile: skip step 1 (contact), show steps 2-4 only
// Logged-in + incomplete profile: show inline profile prompt then steps 2-4
// Logged-out: show sign-in gate
  ...
  return {
    control: (b: any, s: any) => ({
      ...
      boxShadow: s.isFocused
          ? hasError ? "0 0 0 2px rgba(239,68,68,0.4)" : "0 0 0 2px rgba(249,115,22,0.45)"
          : "none",
      cursor: "pointer",
          ...
          {labels.map((label, i) => {
            ...
            return (
                <React.Fragment key={n}>
                  {i > 0 && (
                      <div className={`flex-1 h-0.5 transition-colors duration-300 ${done ? "bg-orange-400" : "bg-charcoal-100"}`} />
                  )}
                  <div className="flex flex-col items-center gap-1.5 flex-shrink-0">
                    ...
                    <span className={`text-[10px] font-semibold hidden sm:block leading-none ${
                        active ? "text-orange-600" : done ? "text-charcoal-400" : "text-charcoal-300"
                    }`}>
                  {label}
                </span>
                  </div>
  ...
  return (
      <div className="max-w-md mx-auto px-4">
        <div className="bg-white rounded-2xl border border-charcoal-100 shadow-sm p-8 text-center">
          ...
          <h2 className="font-display text-xl font-bold text-charcoal-950 mb-2 tracking-tight">
          ...
          <div className="space-y-3">
            <Link href={`/register?redirect=/request-verification`}
                  className="flex items-center justify-center gap-2 w-full bg-orange-500 hover:bg-orange-600 text-white font-semibold text-sm px-6 py-3.5 rounded-xl transition-all hover:shadow-lg hover:shadow-orange-500/20 hover:-translate-y-0.5">
              Create free account
          ...
          <p className="text-xs text-charcoal-400 mt-5 leading-relaxed">
            Free account · No payment until your report is delivered
          </p>
        </div>
      </div>
  );
...
function ProfilePrompt({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        onComplete(phone as string, country);
      } else {
        ...
        <div className="bg-white rounded-2xl border border-charcoal-100 shadow-sm p-8">
          ...
          <div className="flex items-center gap-3 mb-6 pb-5 border-b border-charcoal-100">
            {profile.image ? (
                ...
                <img src={profile.image} alt={profile.name ?? "User"} referrerPolicy="no-referrer"
            ...
            <span className="ml-auto text-[10px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-200 px-2 py-1 rounded-full flex-shrink-0">
            Signed in
          </span>
          </div>

          <div className="mb-2">
            <h2 className="font-display text-lg font-bold text-charcoal-950 tracking-tight">
              Complete your profile
            </h2>
            <p className="text-charcoal-500 text-sm mt-1 leading-relaxed">
              We need your phone number and country to send WhatsApp updates about your project. This only takes a moment.
            </p>
          </div>
          ...
          <div className="space-y-4 mt-5">
            <div>
              ...
              <p className="text-xs text-charcoal-400 mt-1">Used for WhatsApp project updates and inspector coordination.</p>
            </div>
...
// ─── User identity banner (shown at top of form for logged-in users) ──────────
      ...
      <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 mb-6">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-emerald-800 truncate">
            Submitting as {profile.name ?? profile.email}
          </p>
          <p className="text-[10px] text-emerald-600 truncate">{profile.email}</p>
        </div>
        <Link href="/dashboard/settings" className="text-[10px] font-semibold text-emerald-600 hover:text-emerald-800 transition-colors flex-shrink-0">
          Edit profile
        </Link>
      </div>
  );
...
export default function RequestVerificationForm({
  ...
  // Determine step count based on auth state
  // Authenticated + complete profile: 3 steps (location → details → upload)
  // Otherwise: 4 steps (contact → location → details → upload)
  ...
  // For the profile prompt — tracks whether user has just completed their profile
  ...
  // Validators depend on whether contact step is shown
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        phone:            isPreFilled ? (localPhone || userProfile!.phone || undefined) : (form.phone || undefined),
  ...
  // ── Gate: logged in but profile incomplete ────────────────────────────────
  ...
  // ── Success screen ─────────────────────────────────────────────────────────
  if (success) {
    const displayName  = isPreFilled ? userProfile!.name?.split(" ")[0] : form.name.split(" ")[0];
    const displayEmail = isPreFilled ? userProfile!.email : form.email;

    const whatsappShareText = encodeURIComponent(
        `I just submitted a verification request with GRUTH — they send independent inspectors to verify properties and projects in Kenya. Highly recommend if you're managing anything back home from abroad.\n\nhttps://gruth.ke/request-verification`
    );
    ...
    return (
        <div className="max-w-md w-full mx-auto px-4 py-16 text-center">
          ...
          <p className="text-charcoal-600 leading-relaxed mb-2">
            Our team will contact <strong>{displayName}</strong> within <strong>2 business hours</strong> via email and WhatsApp.
          </p>
          <p className="text-charcoal-400 text-sm mb-8">
            Confirmation sent to <span className="font-medium text-charcoal-600">{displayEmail}</span>
          </p>

          {/* Status steps */}
          <div className="grid grid-cols-3 gap-3 bg-white border border-charcoal-100 rounded-2xl p-5 mb-6">
            {[
          ...
          {isAuthenticated && (
              <Link href="/dashboard"
                    className="flex items-center justify-center gap-2 w-full bg-charcoal-950 hover:bg-charcoal-800 text-white text-sm font-semibold px-6 py-3.5 rounded-xl transition-all mb-6">
                View my dashboard →
              </Link>
          )}
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-2xl p-5 mb-6 text-left">
            <p className="font-display font-bold text-charcoal-950 text-sm mb-1">
              Know someone else who needs this?
            </p>
            <p className="text-charcoal-600 text-xs leading-relaxed mb-4">
              Share GRUTH with your community. Every friend you refer who completes a verification earns you a free one.
            </p>
            <div className="flex gap-2">
              <a href={`https://wa.me/?text=${whatsappShareText}`} target="_blank" rel="noreferrer"
                 className="flex-1 inline-flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1db955] text-white text-xs font-bold py-2.5 rounded-xl transition-colors">
              ...
              {isAuthenticated && (
                  <Link href="/dashboard/settings"
                        className="inline-flex items-center justify-center gap-1.5 bg-white border border-charcoal-200 hover:border-orange-300 text-charcoal-700 text-xs font-semibold px-4 py-2.5 rounded-xl transition-all whitespace-nowrap">
                    My referral link
                  </Link>
              )}
            </div>
          ...
          <a href="/" className="inline-flex items-center justify-center text-charcoal-400 hover:text-charcoal-700 text-sm transition-colors">
            ← Back to Home
          </a>
        </div>
    );
  ...
  // Map logical step numbers to content sections
  // isPreFilled: step 1=location, 2=details, 3=upload
  // !isPreFilled: step 1=contact, 2=location, 3=details, 4=upload
      ...
      `}</style>
        ...
        <div className="max-w-xl mx-auto px-4 sm:px-6">
          <div className="bg-white rounded-2xl border border-charcoal-100 shadow-sm p-6 sm:p-8">
            ...
            {/* Identity banner for logged-in users */}
            {isPreFilled && userProfile && <IdentityBanner profile={userProfile} />}

            <form onSubmit={handleSubmit} noValidate>

              {/* ── STEP: Contact (unauthenticated only) ──────────────────── */}
              {isContactStep && (
                  <div className="space-y-4">
                    <div className="mb-5">
                      <h2 className="font-display text-xl font-bold text-charcoal-950 mb-1">Your contact details</h2>
                      <p className="text-charcoal-500 text-sm">We'll use these to send your confirmation and quote.</p>
                    </div>
                    ...
                    <div>
                      ...
                      <input type="text" value={form.name} onChange={e => set("name", e.target.value)}
                             ...
                             placeholder="James Mwangi" autoComplete="name"/>
                    ...
                    <div>
                      ...
                      <div className={`rpi-wrapper ${errors.phone ? "error" : ""}`}>
                        <PhoneInput international defaultCountry="GB" countryCallingCodeEditable={false}
                                    value={form.phone as E164Number} onChange={val => set("phone", val ?? "")}
                                    placeholder="Enter phone number"/>
                      </div>
                    ...
                    <p className="text-xs text-charcoal-400 text-center">
                      ...
                      <Link href="/login?redirect=/request-verification" className="text-orange-600 hover:text-orange-700 font-medium underline">
                        Sign in
                      </Link>{" "}
                      to skip this step.
                    </p>
                  ...
                  <div className="space-y-5">
                    ...
                    <div>
                      ...
                      <Select instanceId="service-select" options={SERVICE_OPTIONS} value={form.serviceOption}
                              onChange={opt => { set("serviceOption", opt ?? null); set("serviceType", opt?.value ?? ""); }}
              ...
              {isUploadStep && (
                  <div className="space-y-5">
                    <div className="flex items-center justify-between">
                      <div>
                        <h2 className="font-display text-xl font-bold text-charcoal-950 mb-1">Supporting files</h2>
                        <p className="text-charcoal-500 text-sm">Attach plans, photos or documents to help your inspector.</p>
                      </div>
                      <BackBtn onClick={back} />
                    </div>
                    ...
                    <p className="text-xs text-charcoal-400">
                      Files are uploaded securely and only shared with your assigned inspector and the GRUTH team.
                    </p>

                    {/* Summary */}
                    <div className="rounded-2xl bg-charcoal-950 p-5">
                      <p className="font-semibold text-white text-sm mb-3">Request summary</p>
                      <div className="space-y-1.5 text-sm">
                        {[
                          ...
                          ...(isPreFilled ? [
                            ["Submitting as", userProfile!.name ?? userProfile!.email],
                          ] : []),
                        ].map(([k, v]) => (
                            <div key={k} className="flex gap-3">
                              <span className="text-charcoal-500 w-28 flex-shrink-0">{k}</span>
                              <span className="text-charcoal-200 truncate">{v}</span>
                            </div>
                        ))}
                        {uploadedFiles.length > 0 && (
                            <div className="flex gap-3">
                              <span className="text-charcoal-500 w-28 flex-shrink-0">Files</span>
                              <span className="text-charcoal-200">{uploadedFiles.length} attached</span>
                            </div>
                        )}
                      </div>
                    </div>
