...
import KenyaLocationPicker, {
  type LocationValue,
} from "@/components/shared/KenyaLocationPicker";
import {
  SERVICES,
  calculatePrice,
  COUNTY_TIERS,
          ...
          return (
            <React.Fragment key={n}>
              ...
              <div className="flex flex-col items-center gap-1.5 flex-shrink-0">
                ...
                <span
                  className={`text-[10px] font-semibold hidden sm:block leading-none ${active ? "text-orange-600" : done ? "text-charcoal-400" : "text-charcoal-300"}`}
                >
                  {label}
                </span>
              </div>
...
function PricePanel({
  ...
  const { baseKes, countySurcharge, urgencySurcharge, totalKes } =
    calculatePrice({
  ...
  const service = SERVICES.find(
    (s) => s.name === serviceType || s.id === serviceType,
  );
    ...
    <div className="rounded-2xl overflow-hidden border border-charcoal-200">
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex-shrink-0 flex flex-col gap-1 items-end">
            ...
            <div className="flex items-center gap-1.5">
              {/* Visa */}
              <div className="bg-[#1A1F71] text-white text-[8px] font-bold px-2 py-0.5 rounded">
                VISA
              </div>
              {/* Mastercard */}
              ...
              {/* M-Pesa */}
              <div className="bg-[#00A651] text-white text-[8px] font-bold px-1.5 py-0.5 rounded">
                M-PESA
              </div>
            </div>
      ...
      <div className="bg-white px-5 py-4 space-y-2.5">
        <div className="flex justify-between text-sm">
          <span className="text-charcoal-600">
            {service?.emoji} {service?.name ?? serviceType}
          </span>
          <span className="font-medium text-charcoal-900">
            KES {baseKes.toLocaleString()}
          </span>
        </div>
        {countySurcharge > 0 && (
          <div className="flex justify-between text-sm">
            ...
            <span className="text-charcoal-700">
              + KES {countySurcharge.toLocaleString()}
            </span>
          </div>
        )}
        {urgencySurcharge > 0 && (
          <div className="flex justify-between text-sm">
            ...
            <span className="text-charcoal-700">
              + KES {urgencySurcharge.toLocaleString()}
            </span>
          </div>
        )}
      ...
      <div className="bg-charcoal-50 border-t border-charcoal-100 px-5 py-3">
        <p className="text-[10px] text-charcoal-400 leading-relaxed">
          Pay by <strong className="text-charcoal-600">card</strong> (Visa,
          Mastercard, Amex),{" "}
          <strong className="text-charcoal-600">M-Pesa</strong> STK push, or{" "}
          <strong className="text-charcoal-600">Apple Pay</strong>. Charged in
          KES. No hidden fees. Report guaranteed or full refund.
        </p>
      </div>
    </div>
  );
    ...
    <div className="max-w-md mx-auto px-4">
      <div className="bg-white rounded-2xl border border-charcoal-100 shadow-sm p-8 text-center">
        ...
        <p className="text-charcoal-500 text-sm leading-relaxed mb-6">
          Create a free account to submit your verification request, track
          progress in your dashboard, and receive notifications when your report
          is ready.
        </p>
        ...
        <div className="space-y-3">
          <Link
            href="/register?redirect=/request-verification"
          ...
          <Link
            href="/login?redirect=/request-verification"
  ...
  return (
    <div className="max-w-md mx-auto px-4">
      <div className="bg-white rounded-2xl border border-charcoal-100 shadow-sm p-8">
        ...
        <h2 className="font-display text-lg font-bold text-charcoal-950 mb-1 tracking-tight">
          Complete your profile
        </h2>
        <p className="text-charcoal-500 text-sm leading-relaxed mb-5">
          We need your phone and country to send WhatsApp updates.
        </p>
  ...
  return (
    <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-3 mb-6">
      ...
      <Link
        ...
        className="text-[10px] font-semibold text-emerald-600 hover:text-emerald-800 transition-colors flex-shrink-0"
      >
        Edit profile
      </Link>
    </div>
...
export default function RequestVerificationForm({
  ...
  const [form, setFormState] = useState<FormState>({
    ...
    phone: (userProfile?.phone ?? userProfile?.whatsapp ?? "") as
      | E164Number
      | "",
    projectLocation: "",
    county: "",
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setErrors({
          submit: data.error ?? "Payment setup failed. Please try again.",
        });
        setLoading(false);
        return;
      }
    ...
    <>
      ...
      <div className="max-w-xl mx-auto px-4 sm:px-6">
        <div className="bg-white rounded-2xl border border-charcoal-100 shadow-sm p-6 sm:p-8">
          ...
          {isPreFilled && userProfile && (
            <IdentityBanner profile={userProfile} />
          )}
              ...
              <div className="space-y-4">
                <div className="mb-5">
                  ...
                  <p className="text-charcoal-500 text-sm">
                    We'll use these to send your confirmation and updates.
                  </p>
                </div>
                <div>
            ...
            {isLocationStep && (
              <div className="space-y-5">
                  ...
                  {form.serviceType && (
                    <div className="mt-2 flex items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-orange-600">
                        KES{" "}
                        {(
                          SERVICES.find((s) => s.name === form.serviceType)
                            ?.baseKes ?? 0
                        ).toLocaleString()}
                      </span>
                      <span className="text-xs text-charcoal-400">
                        · final price on next step
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div>
                  ...
                  {form.county &&
                    COUNTY_TIERS[form.county] &&
                    COUNTY_TIERS[form.county] !== "major" && (
                      <p className="text-xs text-charcoal-400 mt-1">
                        {form.county} is a {COUNTY_TIERS[form.county]} county —
                        KES{" "}
                        {COUNTY_TIER_SURCHARGE[
                          COUNTY_TIERS[form.county]!
                        ].toLocaleString()}{" "}
                ...
                <div>
                  ...
                  {form.urgency === "urgent" && (
                    <p className="text-xs text-amber-600 mt-1.5">
                      ⚡ 48-hour premium: KES{" "}
                      {URGENCY_SURCHARGE.urgent.toLocaleString()} added
                    </p>
                  )}
                </div>
                <div>
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-charcoal-500 text-sm">
                      Attach supporting files, then pay to submit.
                    </p>
                  </div>
                  <BackBtn onClick={back} />
                </div>
                  ...
                  <div className="space-y-1.5 text-sm">
                    {[
                      ...
                      <div key={k} className="flex gap-3">
                        <span className="text-charcoal-400 w-28 flex-shrink-0">
                          {k}
                        </span>
                        <span className="text-charcoal-700 truncate font-medium">
                          {v}
                        </span>
                      </div>
                    ))}
                ...
                <div className="flex items-center justify-center gap-3 flex-wrap">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="bg-[#1A1F71] text-white text-[9px] font-bold px-2 py-0.5 rounded">
                      VISA
                    </div>
                    ...
                    <div className="bg-[#00A651] text-white text-[9px] font-bold px-2 py-0.5 rounded">
                      M-PESA
                    </div>
                ...
                <p className="text-xs text-charcoal-400 text-center leading-relaxed">
                  You'll be redirected to Paystack's secure checkout. Your
                  request is only submitted after payment is confirmed.{" "}
                  <a
                    href="/privacy"
