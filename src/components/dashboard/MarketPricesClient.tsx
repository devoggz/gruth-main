  ...
  return (
    ...
    >
      <div className="bg-charcoal-950 text-white rounded-2xl px-4 py-3.5 text-xs">
        {/* Material name */}
        <p className="font-semibold text-white text-sm mb-2 leading-tight">{row.materialName}</p>

        {/* Price range */}
        {row.priceLow != null && row.priceHigh != null ? (
          <div className="mb-2.5">
            <p className="text-charcoal-400 text-[10px] uppercase tracking-widest font-semibold mb-1">Market range</p>
            <div className="flex items-center gap-2">
              <span className="text-emerald-400 font-bold">KES {row.priceLow.toLocaleString()}</span>
              <span className="text-charcoal-600">–</span>
              <span className="text-red-400 font-bold">KES {row.priceHigh.toLocaleString()}</span>
            </div>
            {/* Range bar */}
            <div className="mt-1.5 h-1 bg-charcoal-800 rounded-full overflow-hidden">
              {(() => {
                const range = row.priceHigh - row.priceLow;
                const midPct = range > 0 ? ((row.priceKes - row.priceLow) / range) * 100 : 50;
                return (
        ...
        {/* Description */}
        {row.description && (
          <p className="text-charcoal-300 text-[11px] leading-relaxed mb-2.5 border-t border-charcoal-800 pt-2.5">{row.description}</p>
        )}
        ...
        <div className="flex items-center justify-between gap-2 border-t border-charcoal-800 pt-2">
          <span className="text-charcoal-400 text-[10px] truncate">
            {row.sourceUrl
              ? <a href={row.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-orange-400 hover:text-orange-300 pointer-events-auto">{row.sourceName}</a>
              : row.sourceName}
          </span>
          <span className="text-charcoal-500 text-[10px] flex-shrink-0">{formatRelativeDate(row.updatedAt)}</span>
        </div>
      </div>
      {/* Arrow */}
  ...
  return (
    ...
    >
      ...
      <td className="px-5 py-3 text-charcoal-400 text-xs hidden lg:table-cell">
        {row.sourceUrl ? (
          <a href={row.sourceUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:text-blue-700 hover:underline transition-colors">
            {row.sourceName}
          </a>
        ) : row.sourceName}
      </td>
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="font-medium text-charcoal-900 text-sm leading-snug">{row.materialName}</div>
      ...
      {expanded && (
        <div className="mt-3 pt-3 border-t border-charcoal-100 space-y-1.5 text-left">
          {row.description && (
            <p className="text-xs text-charcoal-500 leading-relaxed">{row.description}</p>
          )}
...
export function MarketPricesClient({
  counties, defaultCountyId, defaultPrices, categories, allCountyPrices,
}: Props) {
  ...
  const [comparisonMaterial, setComparisonMaterial] = useState<string | null>(null);
  const [view, setView] = useState<"table" | "chart">("table");
  const countyScrollRef = useRef<HTMLDivElement>(null);

  const fetchPrices = useCallback(async (countyId: string) => {
    if (countyId === defaultCountyId) { setPrices(defaultPrices); return; }
    setLoading(true);
    try {
  ...
  useEffect(() => { fetchPrices(selectedCounty); }, [selectedCounty, fetchPrices]);
  ...
  const comparisonData = comparisonMaterial
    ? counties.map((county) => {
        ...
        const match = countyPrices.find((p) => p.materialName === comparisonMaterial);
        return { county: county.name, price: match?.priceKes ?? 0, low: match?.priceLow ?? 0, high: match?.priceHigh ?? 0 };
      }).filter((d) => d.price > 0)
    : [];

  const allMaterials = Array.from(new Set(defaultPrices.map((p) => p.materialName))).sort();
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-widest mb-2.5">County</p>
        <div ref={countyScrollRef} className="flex gap-2 overflow-x-auto pb-1 scrollbar-none" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
          {counties.map((c) => (
            <button
              key={c.id}
        ...
        <div className="flex items-center bg-charcoal-100 rounded-xl p-1 gap-1">
          <button
            ...
            className={[
              ...
              view === "table" ? "bg-white text-charcoal-950 shadow-sm" : "text-charcoal-500 hover:text-charcoal-700",
            ].join(" ")}
          >
            Price List
          </button>
          <button
            ...
            className={[
              "px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150",
              view === "chart" ? "bg-white text-charcoal-950 shadow-sm" : "text-charcoal-500 hover:text-charcoal-700",
            ].join(" ")}
          >
            Compare Counties
          </button>
      ...
      {view === "chart" && (
        <div className="card p-5 sm:p-6 space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            <div>
              <h3 className="font-display font-semibold text-charcoal-950 text-base">County Price Comparison</h3>
              <p className="text-xs text-charcoal-400 mt-0.5">Select a material to compare its price across all available counties</p>
            </div>
            <select
          ...
          {!comparisonMaterial ? (
            <div className="flex flex-col items-center justify-center h-48 text-charcoal-300 gap-3">
              <svg className="w-10 h-10 text-charcoal-200" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                <path d="M3 3v18h18" /><path d="M7 16l4-4 4 4 4-6" />
              </svg>
              <p className="text-sm text-charcoal-400">Pick a material above to see price comparison</p>
            </div>
          ) : comparisonData.length === 0 ? (
            <div className="flex items-center justify-center h-48 text-charcoal-400 text-sm">No data available for this material</div>
          ) : (
            <>
              {/* Min / avg / max callouts */}
              {(() => {
                const sorted = [...comparisonData].sort((a, b) => a.price - b.price);
                const cheapest = sorted[0];
                const priciest = sorted[sorted.length - 1];
                const avg = Math.round(comparisonData.reduce((s, d) => s + d.price, 0) / comparisonData.length);
                return (
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3">
                      <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">Cheapest</p>
                      <p className="font-display font-bold text-emerald-700 text-lg mt-0.5 leading-none">KES {cheapest.price.toLocaleString()}</p>
                      <p className="text-xs text-emerald-500 mt-1">{cheapest.county}</p>
                    </div>
                    <div className="bg-charcoal-50 border border-charcoal-100 rounded-xl p-3">
                      <p className="text-[10px] font-bold text-charcoal-500 uppercase tracking-widest">Average</p>
                      <p className="font-display font-bold text-charcoal-800 text-lg mt-0.5 leading-none">KES {avg.toLocaleString()}</p>
                      <p className="text-xs text-charcoal-400 mt-1">{comparisonData.length} counties</p>
                    </div>
                    <div className="bg-red-50 border border-red-100 rounded-xl p-3">
                      <p className="text-[10px] font-bold text-red-500 uppercase tracking-widest">Priciest</p>
                      <p className="font-display font-bold text-red-600 text-lg mt-0.5 leading-none">KES {priciest.price.toLocaleString()}</p>
                      <p className="text-xs text-red-400 mt-1">{priciest.county}</p>
                    </div>
                  </div>
                );
              })()}
              ...
              <div className="h-60 sm:h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={comparisonData} margin={{ top: 4, right: 4, left: 0, bottom: 4 }} barCategoryGap="28%">
                    <CartesianGrid strokeDasharray="3 3" stroke="#f0eeeb" vertical={false} />
                    <XAxis dataKey="county" tick={{ fontSize: 11, fill: "#75736c" }} axisLine={false} tickLine={false} />
                    <YAxis
                      tick={{ fontSize: 11, fill: "#75736c" }} axisLine={false} tickLine={false} width={44}
                      tickFormatter={(v) => v >= 1000 ? `${(v / 1000).toFixed(1)}k` : String(v)}
                    />
                    <Tooltip content={<ComparisonTooltip />} cursor={{ fill: "#f97316", opacity: 0.06 }} />
                    <Bar dataKey="price" radius={[6, 6, 0, 0]} maxBarSize={52}>
                      {comparisonData.map((entry) => (
                        <Cell
                          key={entry.county}
                          fill={entry.county === selectedCountyName ? "#f97316" : "#e8e6e2"}
                        />
                      ))}
                    </Bar>
                  </BarChart>
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            <div className="relative w-full sm:max-w-xs">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-charcoal-300 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
              ...
              />
              {search && (
                <button onClick={() => setSearch("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-charcoal-300 hover:text-charcoal-600 transition-colors">
          ...
          {loading ? (
            ...
            <div className="card p-12 text-center">
              ...
              <p className="text-charcoal-500 text-sm font-medium">No prices found{search ? ` for "${search}"` : ""}</p>
              <p className="text-charcoal-400 text-xs mt-1">Try a different search term or category</p>
            </div>
          ) : (
            <div className="space-y-5">
              {Object.entries(grouped).map(([category, rows]) => (
                <div key={category} className="card overflow-hidden">
                  ...
                  <div className="px-5 py-3 bg-charcoal-50/80 border-b border-charcoal-100 flex items-center justify-between">
                    <h3 className="font-display font-semibold text-charcoal-900 text-sm">{category}</h3>
                    <span className="text-[11px] text-charcoal-400 font-medium">{rows.length} {rows.length === 1 ? "item" : "items"}</span>
                  </div>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-charcoal-100">
                          <th className="px-5 py-2.5 text-left text-[10px] font-bold text-charcoal-400 uppercase tracking-widest">Material</th>
                          <th className="px-5 py-2.5 text-left text-[10px] font-bold text-charcoal-400 uppercase tracking-widest">Unit</th>
                          <th className="px-5 py-2.5 text-right text-[10px] font-bold text-charcoal-400 uppercase tracking-widest">Price (KES)</th>
                          <th className="px-5 py-2.5 text-left text-[10px] font-bold text-charcoal-400 uppercase tracking-widest">Trend</th>
                          <th className="px-5 py-2.5 text-left text-[10px] font-bold text-charcoal-400 uppercase tracking-widest hidden lg:table-cell">Source</th>
                          <th className="px-5 py-2.5 text-left text-[10px] font-bold text-charcoal-400 uppercase tracking-widest hidden lg:table-cell">Updated</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-charcoal-50/80">
                        {rows.map((row) => <PriceRowItem key={row.id} row={row} />)}
                      </tbody>
                    </table>
                  </div>
                  ...
                  <div className="sm:hidden divide-y divide-charcoal-100">
                    {rows.map((row) => <MobilePriceCard key={row.id} row={row} />)}
                  </div>
                </div>
              ))}
            </div>
      ...
      <p className="text-[11px] text-charcoal-400 leading-relaxed border-t border-charcoal-100 pt-4">
        <strong className="font-semibold text-charcoal-500">Disclaimer:</strong> Prices are indicative mid-market rates sourced from supplier data and GRUTH field surveys.
        Actual prices vary by quantity, supplier, and market conditions. Price ranges shown represent typical supplier quotes at time of survey.
      </p>
    </div>
  );
}
