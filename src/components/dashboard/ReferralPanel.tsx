...
export default function ReferralPanel({
                                          userId, userName, referralCode: initialCode,
                                          totalReferred, unusedCredits, appUrl,
                                      }: Props) {
    ...
    return (
        <div className="bg-white border border-charcoal-100 rounded-2xl overflow-hidden">
            ...
            <div className="bg-charcoal-950 px-5 py-5">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-charcoal-400 text-xs mt-1.5 leading-relaxed max-w-xs">
                            Every time someone signs up using your link and completes their first verification, you earn one free verification credit.
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-5">
                    {[
                        ...
                        { label: "Free credits",   value: unusedCredits,  color: "text-orange-400"  },
                    ].map(({ label, value, color }) => (
                        <div key={label} className="bg-white/[0.05] border border-white/[0.08] rounded-xl px-4 py-3 text-center">
                            <div className={`font-display text-2xl font-bold ${color}`}>{value}</div>
                            <div className="text-[10px] font-bold text-charcoal-500 uppercase tracking-widest mt-0.5">{label}</div>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="p-5 space-y-4">
                <div>
                    <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-widest mb-2">Your referral link</p>
                    {loading ? (
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={copy}
                                ...
                                {copied ? (
                                    <><svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M20 6L9 17l-5-5"/></svg>Copied</>
                                ) : (
                ...
                <div className="border-t border-charcoal-100 pt-4">
                    <p className="text-[10px] font-bold text-charcoal-400 uppercase tracking-widest mb-3">How it works</p>
                    <div className="space-y-2.5">
                        {[
                            { step: "1", text: "Share your referral link with friends or family planning a project in Kenya" },
                            { step: "2", text: "They sign up and complete their first verification" },
                            { step: "3", text: "You get a free verification credit — automatically applied to your next order" },
                        ].map(({ step, text }) => (
                ...
                <span className="w-5 h-5 bg-orange-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                  {step}
                                ...
                                <p className="text-xs text-charcoal-600 leading-relaxed">{text}</p>
                            </div>
                        ))}
                    </div>
                </div>
