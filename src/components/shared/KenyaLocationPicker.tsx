...
const KENYA_BOUNDS = {
    ...
    ne:     { lat:  4.62, lng: 41.90 },
    center: { lat:  0.02, lng: 37.90 },
    zoom:   6,
};
...
function LeafletMapInner({
    ...
    return (
        <div
            ref={containerRef}
            style={{ height: "280px", width: "100%", borderRadius: "0.75rem", overflow: "hidden" }}
        />
    );
}
    ...
    useEffect(() => {
        ...
        timerRef.current = setTimeout(async () => {
            ...
            try {
                ...
                setResults(data.filter((r: NominatimResult) => r.address?.country_code === "ke"));
            } catch {
                setResults([]);
            } finally {
                setLoading(false);
...
export default function KenyaLocationPicker({ value, onChange, error, placeholder }: Props) {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!v) onChange({ address: "", lat: KENYA_BOUNDS.center.lat, lng: KENYA_BOUNDS.center.lng });
    };
        ...
        <div className="space-y-2">
            ...
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                    ...
                    <input
                        ...
                        onFocus={() => { if (inputText.length >= 3) setShowDropdown(true); }}
                        ...
                        {inputText && !loading && !reversing && (
                            <button type="button"
                                    onClick={() => {
                                        setInputText("");
                                        setShowDropdown(false);
                                        onChange({ address: "", lat: KENYA_BOUNDS.center.lat, lng: KENYA_BOUNDS.center.lng });
                ...
                {showDropdown && (results.length > 0 || loading) && (
                    <div className="absolute left-0 right-0 top-full mt-1 z-50 bg-white border border-charcoal-200 rounded-xl shadow-xl overflow-hidden">
                        ...
                        {results.map((r, i) => (
                            ...
                            >
                                <svg className="w-3.5 h-3.5 text-orange-500 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                                    <path d="M17.657 16.657L13.414 20.9a2 2 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
                                    <path d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                                </svg>
                                <div className="min-w-0">
                                    <p className="text-sm font-medium text-charcoal-900 truncate">{shorten(r.display_name)}</p>
                                    <p className="text-xs text-charcoal-400 truncate mt-0.5">{r.display_name}</p>
                                </div>
                            </button>
                        ))}
                    </div>
          ...
          `}</style>
                    ...
                    <div className="bg-charcoal-50 border-t border-charcoal-100 px-3 py-2 flex items-center gap-2">
                        <svg className="w-3.5 h-3.5 text-orange-500 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
