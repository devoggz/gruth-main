...
import { scrapeSandPrices    } from "./scrapeSandPrices";
import { scrapeTimberPrices  } from "./scrapeTimberPrices";
import { scrapeFinishesPrices} from "./scrapeFinishesPrices";
import { scrapeHardwarePrices} from "./scrapeHardwarePrices";
...
const UNIT_KEYWORDS: Array<{ keywords: string[]; unit: string }> = [
  ...
  { keywords: ["tonne", "ton", "sand", "hardcore", "quarry dust", "ballast"],   unit: "per tonne" },
  { keywords: ["sheet", "plywood", "brc mesh"],                                 unit: "per sheet" },
  { keywords: ["piece", "block", "lintel", "hinge", "padlock", "pipe", "mcb"],  unit: "per piece" },
...
function deduplicate(prices: NormalisedPrice[]): NormalisedPrice[] {
  ...
  for (const p of prices) {
    ...
    const existingHasRange = existing.priceLow != null && existing.priceHigh != null;
...
const SCRAPERS: Array<{ name: string; fn: () => Promise<RawScrapedPrice[]> }> = [
  ...
  { name: "sand",     fn: scrapeSandPrices     },
  { name: "timber",   fn: scrapeTimberPrices   },
  { name: "finishes", fn: scrapeFinishesPrices },
  { name: "hardware", fn: scrapeHardwarePrices },
];
...
export async function runScrapers(): Promise<ScraperRunResult> {
  ...
  const prices: NormalisedPrice[] = withFlags.map(({ anomaly: _a, zScore: _z, ...rest }) => rest);
